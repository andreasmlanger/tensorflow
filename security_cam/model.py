...
if not os.path.exists(path_to_weights_file):
	...
	urllib.request.urlretrieve(URL_TO_WEIGHTS_FILE, path_to_weights_file)
...
def create_inception_resnet_v2():
	...
	up = Lambda(scaling, output_shape=tf.keras.backend.int_shape(up)[1:], arguments={'scale': 0.17})(up)
	x = add([x, up])
	...
	up = Lambda(scaling, output_shape=tf.keras.backend.int_shape(up)[1:], arguments={'scale': 0.1})(up)
	x = add([x, up])
	...
	up = Lambda(scaling, output_shape=tf.keras.backend.int_shape(up)[1:], arguments={'scale': 0.2})(up)
	x = add([x, up])
	...
	up = Lambda(scaling, output_shape=tf.keras.backend.int_shape(up)[1:], arguments={'scale': 1})(up)
	x = add([x, up])
...
face_encoder = create_inception_resnet_v2()

# Load the weights of the model
face_encoder.load_weights(path_to_weights_file)
